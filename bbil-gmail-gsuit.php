...
/*
Send Email With Gmail API For Contact Form 7
*/
function wpcf7_bgg_send_gmail_api( $cf7 ) {
  global $messages;
  $messages = $cf7->messages;
  function bgg_customize_wp_mail($args){
    if ( isset( $args['to'] ) ) {
      $to = $args['to'];
    }
    ...
    try{
      ...
      function customize_cf7_success_status($result){
        global $messages;
      ...
      add_filter('wpcf7_ajax_json_echo','customize_cf7_success_status');
    }catch (\Exception $exception){
      $exception->getMessage();

      function customize_cf7_failed_status($result){
        global $messages;
      ...
      add_filter('wpcf7_ajax_json_echo','customize_cf7_failed_status');
    }
  }

  add_filter('wp_mail','bgg_customize_wp_mail');
}

$bgg_settings_value = get_option( 'bgg_settings_value');
if ($bgg_settings_value == 1) {
  add_action("wpcf7_before_send_mail", "wpcf7_bgg_send_gmail_api");
}
